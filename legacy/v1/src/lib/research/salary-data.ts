...
const GENERIC_ENTRY: SalaryEntry = {
  keywords: [],
...
export function lookupSalary(jobTitle: string): SalaryResearch {
  ...
  const matched = SALARY_MAP.find((entry) =>
    entry.keywords.some((keyword) => titleLower.includes(keyword) || keyword.includes(titleLower))
  );

  const entry = matched || GENERIC_ENTRY;

  // Determine which benefits set is more relevant based on title context
  // Both are included since the briefing covers NZ and AU
  const commonBenefits = [
    "NZ: " + NZ_BENEFITS.slice(0, 5).join(", "),
    "AU: " + AU_BENEFITS.slice(0, 5).join(", "),
  ];
